...
export function codexSelectionView(
    ...
    return `
      <article class="panel codex-authorize">
        <div>
          ...
          <p>Token Saver does not scan Codex conversations automatically. Folder selection is temporary and stays in this running application.</p>
        </div>
  ...
  const importCount = Math.min(10, preview.eligibleCount, preview.items.length);
  return `
    <article class="panel codex-authorize ready">
      ...
      <div class="codex-preview-metrics">
        ...
        <span><strong>${preview.eligibleCount}</strong><small>within size limit</small></span>
        <span><strong>${bytes(preview.totalSizeBytes)}</strong><small>folder total</small></span>
      ...
      <small class="codex-limit-note">Import limit: 10 files, 10 MB per file, 25 MB total. Oversized files remain visible but are not read.</small>
    </article>`;
}
