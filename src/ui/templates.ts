import type { AgentSession, Finding, FindingType, Integration, ViewId, WorkspaceState } from "../types";
...
function verifiedTotals(state: WorkspaceState): {
  records: number;
...
function overviewHeadline(state: WorkspaceState): { title: string; detail: string; tone: string } {
  ...
  if (!detected) return {
    ...
    detail: "Scan your existing AI tools, then review one recommended setup instead of configuring every engine by hand.",
    tone: "idle",
  };
  if (!connected) return {
    title: `${detected} AI tool${detected === 1 ? "" : "s"} found`,
    detail: "Detection is complete. Connector setup remains off until you approve it.",
    tone: "ready",
  };
  return {
    title: mode === "automatic" ? "Automatic optimization mode is on" : "Manual strategy control is on",
    detail: `${connected} connected tool${connected === 1 ? "" : "s"}. ${mode === "automatic" ? "Token Saver will prefer compatible low-risk strategies." : "You decide which strategies are allowed."}`,
    tone: "active",
  };
}
...
export function dashboardView(state: WorkspaceState): string {
  if (!state.sessions.length) {
    const detected = state.integrations.filter((item) => item.detected).length;
    return `<div class="product-onboarding"><div class="onboarding-mark">${controlNodeMark()}</div><span class="eyebrow">ONE SIMPLE SETUP</span><h2>Reduce token usage across the AI tools you already use.</h2><p>Token Saver connects compatible optimization engines, applies them quietly, and measures the result. You keep using Codex, Claude Code, Cursor, or OpenCode as usual.</p><div class="onboarding-actions"><button class="button primary" id="empty-scan">${detected ? "Review detected tools" : "Scan for AI tools"}</button><button class="button ghost" id="demo-button">See sample results</button></div><button class="text-button onboarding-import" id="empty-import">Import existing session data</button><small class="onboarding-note">Automatic mode is the default. Advanced users can take control in Strategy Hub.</small></div>`;
  }
...
function integration(item: Integration): string {
  ...
  const detail = item.connected
    ? "Connector is authorized for local usage and session events."
    : item.detected
      ? "Application found. One-time connector approval is still required."
      : item.detail;
...
export function integrationsView(state: WorkspaceState): string {
  const detected = state.integrations.filter((item) => item.detected).length;
  const connected = state.integrations.filter((item) => item.connected).length;
  return `<div class="integration-hero"><div><span class="eyebrow">ONE-TIME SETUP</span><h2>${connected ? `${connected} AI tool${connected === 1 ? "" : "s"} connected` : detected ? `${detected} AI tool${detected === 1 ? "" : "s"} ready for setup` : "Find the AI tools already on this computer"}</h2><p>Token Saver packages client-specific connectors behind one setup flow. Detection never grants conversation access or changes configuration by itself.</p></div><button class="button primary" id="integration-scan">Scan again</button></div><article class="permission-note"><strong>Simple outside, explicit inside.</strong><span>Every connector will show what it reads, what it changes, and how to disconnect it.</span></article><div class="integration-grid">${state.integrations.map(integration).join("")}</div>`;
}
